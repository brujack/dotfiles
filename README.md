...
| Type             | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `setup_user`     | Sets up user environment: configs, symlinks, shell, directory structure, installs git hooks across personal repos, installs GNU make on macOS, registers the Claude marketplaces and installs the enabled plugins declared in ai-config's `settings.json` (see [ADR-0034](docs/adr/0034-claude-plugins-from-ai-config-settings.md)), and — **on the Studio only** — installs two weekly LaunchAgents that summon dependency triage (see [ADR-0024](docs/adr/0024-launchagent-for-the-renovate-held-cadence.md)) |
...
- `--claude-only` — reconcile and update Claude plugins only (with `-t update`). It skips the `ai-config` pull a full run does first, so it reconciles against whatever that checkout already has on disk
