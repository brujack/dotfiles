...
- `test` job: installs a pinned, checksum-verified shellcheck plus bats, runs `make test`, then verifies test count ≥ 840 (regression proxy; 1656 tests, CI-measured 2026-09-05 on `933df693`, run 33978138616)
...
**`_AWS_KEY_PATH`'s default now comes from `DOTFILES_REPO_ROOT` (`lib/constants.sh`), and the reason is that the previous derivation was cwd-sensitive.** Both consumers computed it inline as `$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/keys/aws-cli-team.asc`. `setup_env.sh:48` sources `lib/developer.sh` by a **relative** path, so `BASH_SOURCE[0]` is `./lib/developer.sh` whenever the entry point is invoked as `./setup_env.sh` — and `update_aws_cli` `cd`s to `${HOME}/software_downloads/awscli` (`lib/developer.sh:167`, `:196`) before calling the verifier. From there `cd ./lib/..` fails, `&&` short-circuits, the command substitution returns **empty**, and the key resolves to `/keys/aws-cli-team.asc`. Measured 2026-09-07: `setup_env.sh -t update` reported `[FAIL] aws exit 1` on every run, with `gpg: can't open '/keys/aws-cli-team.asc'` and a retry that failed identically. `DOTFILES_REPO_ROOT` resolves the same expression at **source time**, which is the only moment a relative `BASH_SOURCE[0]` is guaranteed to mean anything; its `${VAR:-}` self-guard stops a re-source from a changed cwd overwriting a correct value.
