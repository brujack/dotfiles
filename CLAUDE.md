...
- `test` job: installs a pinned, checksum-verified shellcheck plus bats, runs `make test`, then verifies test count ≥ 840 (regression proxy; 1622 tests, measured locally via `make test` on `ed59c9a6` — local, pending CI, no CI run for this branch yet; supersedes the 1659-test figure from `bb7b6ab`/#257 as the current reading, lower because this branch deletes tests for the unwired units it removes)
...
- **Overall: 91.83%** (3542/3857 commands, 1622 tests, 18 heuristic disagreements) as measured locally via `bash scripts/run-bash-coverage.sh` on a worktree carrying this branch's deletions at `8c80aa86` — **local, pending CI; there is no CI run for this branch yet**, so this is a preview, not the figure the gate will read. Every commit on this branch since `8c80aa86` is docs-only and outside the instrumented set, so the figure is unchanged at the branch's current tip. It supersedes `bb7b6ab`/#257's figure above as the most recent reading taken. Both numerator and denominator dropped (3595/3934 -> 3542/3857) and the instrumented file count moved 38 -> 37, consistent with this branch deleting the unwired units and their tests rather than any coverage regression. Disagreements moved 23 -> 18. Once this branch's CI run completes, replace this bullet's status rather than appending a further "local" reading over it — the convention in this section is CI's figure, and a second local preview would only restate the same caveat.
