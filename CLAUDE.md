...
- **Overall: 91%** (3439/3755 commands, 1551 tests, 20 heuristic disagreements) as measured by CI on `ubuntu-latest` for `80d853d` (#249) — the figure the gate actually reads. All four verified from run 33225773317: `TOTAL 3439 3755 91%` and `total: 20` in job 99029128139, `Test count: 1551` in job 99029128123. Read all four from **one** run: earlier revisions carried a ratio and a disagreement count taken from different runs, and #248's revision updated the figures while leaving the run and job IDs pointing at #239's — the warning against that was already in this line and did not prevent it; re-reading the line did. **#249 moved the denominator and the prediction that it would not was wrong.** Its six new tests are all in a `.bats` file and 0 of the 34 instrumented files are `.bats`, which is true and was read as "the denominator cannot change" — but `scripts/cadence-notify.sh` is itself instrumented as a tracked `scripts/*.sh`, and the change added ~13 code lines to it. +10 coverable and +10 covered, every new line exercised, ratio unchanged at 91%. The rule that survives: a `.bats`-only change cannot move the denominator, and almost no change is `.bats`-only — check whether the diff also touches an instrumented file before predicting. Gated in CI at **91%** (`bash-coverage` job, blocks auto-merge on drop). A percentage without its denominator is not a coverage figure — report both.
