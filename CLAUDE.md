...
- `test` job: installs a pinned, checksum-verified shellcheck plus bats, runs `make test`, then verifies test count >= 840 (regression proxy; 1626 tests, CI-measured 2026-09-10 on `1d309276`, run 34520947214, job 103018029320; that branch SHA was squash-merged and is no longer an ancestor of master — it landed as `ff66c68e`, #260)
...
- **Overall: 91%** (3454/3772 commands, 1601 tests, 21 heuristic disagreements) as measured by CI on `ubuntu-latest` for `b5e01e6b` (#251) — the figure the gate actually reads. All four read from run 33554914623 (`bash-coverage`, job 100013254264) in a single pass: `TOTAL 3454 3772 91%`, `total: 21`, `Test count: 1601`. Read all four from **one** run: earlier revisions carried a ratio and a disagreement count taken from different runs, and #248's revision updated the figures while leaving the run and job IDs pointing at #239's — the warning against that was already in this line and did not prevent it; re-reading the line did. **#250 is the second measured counter-example to "local reads one point higher", and it is a cleaner one than #244.** Local measured `3442/3755` and CI `3442/3758` — the **numerator is identical** and only the denominator moved, by three lines. The mechanism is the one #244 recorded: `update_aws_cli` and `update_rust` gained platform-conditional guards whose Linux branches CI executes and macOS never reaches, so the coverable set grows on CI alone. The rule stands as a prior, not a law: publish CI's figure, and expect the gap to close rather than widen whenever a change adds a platform-conditional branch. Gated in CI at **91%** (`bash-coverage` job, blocks auto-merge on drop). A percentage without its denominator is not a coverage figure — report both.
- **Overall: 91%** (3549/3883 commands, 1641 tests, 22 heuristic disagreements) as measured by CI on `ubuntu-latest` for `1a832a72` (#252) — this supersedes `b5e01e6b`'s figure above as the current reading. All four read from run 33797748409 (`bash-coverage`, job 100789422426) in a single pass: `TOTAL 3549 3883 91%`, `total: 22`, `Test count: 1641`. **#252 is the third measured counter-example to "local reads one point higher", and the first where CI's numerator is LOWER than local's.** Local measured `3551/3880`, CI `3549/3883` — CI covered 2 fewer commands over 3 more, both displaying 91%. The mechanism is the one #244 and #250 recorded, now running in both directions at once: the change added platform-conditional branches on both sides (a Linux-only GPG path macOS never reaches, a macOS-only `pkgutil` path Linux never reaches), so each platform gains coverable lines the other cannot execute. `lib/developer.sh` grew 216 -> 300 coverable lines, the largest single contributor to the denominator change. Treat the rule as a prior that a platform-symmetric change can invert, not only narrow.
- **Overall: 91%** (3593/3932 commands, 1655 tests, 23 heuristic disagreements) as measured by CI on `ubuntu-latest` for `917da4da` (#254) — this supersedes `1a832a72`'s figure above as the current reading. All four read from run 33939971767 (`bash-coverage`, job 101235329852) in a single pass: `TOTAL 3593 3932 91%`, `total: 23`, `Test count: 1655`. **No local preview was taken for this commit, so it is neither a confirmation nor a counter-example to the local-reads-one-point-higher rule** — recorded as absent rather than left to be inferred from the missing comparison, since every bullet above carries one. The denominator grew 3883 -> 3932 (+49) on a change that **deleted** a production line: `scripts/check-lib-exit-traps.sh` is a new tracked `scripts/*.sh` file and joins the instrumented set by the existing predicate, and `tests/setup_env/run_tmpdir_traps.bats` executes more of `lib/workflows.sh` than the removed save/restore blocks did. Disagreements moved 22 -> 23.
- **Overall: 91%** (3593/3932 commands, 1656 tests, 23 heuristic disagreements) as measured by CI on `ubuntu-latest` for `933df693` (#255) — this supersedes `917da4da`'s figure above as the current reading, and all four are read from run 33978138616 (`bash-coverage`, job 101338368174; `Test count` from job 101338368160) in a single pass. **The coverage triple is byte-identical to #254's — `3593 3932 91%` with 23 disagreements — and that is the expected result, not a copied figure.** #255 adds one test that asserts a fixture (`_RHN_LOCAL_CFG` is non-empty, under `BATS_TEST_TMPDIR`, not the repo path, absent) and executes no production line that was not already traced, so the numerator and denominator both stand still while the test count moves 1655 -> 1656. A test can be worth having and move coverage not at all; the two figures answer different questions. No local preview was taken, so this is again neither a confirmation nor a counter-example to the local-reads-one-point-higher rule.
- **Overall: 91%** (3595/3934 commands, 1659 tests, 23 heuristic disagreements) as measured by CI on `ubuntu-latest` for `bb7b6ab` (#257, merged as `16e568e`; the branch SHA no longer resolves on any branch) — this supersedes `933df693`'s figure above as the current reading. All four read from run 34148440502 (`bash-coverage`, job 101825390439; `Test count` from job 101825390106) in a single pass: `TOTAL 3595 3934 91%`, `total: 23`, `Test count: 1659`. **#257 is the fourth counter-example to "local reads one point higher", and a shape none of the earlier three had: the denominators are IDENTICAL and CI's numerator is 12 LOWER.** Local measured `3607/3934` on the Linux 7950X — same OS family as the runner, so the platform-conditional mechanism #244, #250 and #252 recorded cannot explain it. The coverable set did not move at all; 12 commands executed locally and not in CI. **The mechanism is not established** — the plausible candidate is tool presence rather than platform (the workstation has `ledger` at `~/.local/bin` and a populated `~/.gnupg`, so guard branches reachable there are not reachable on a bare runner), but that was not measured and should not be read as a finding. What is established: a local preview can exceed CI's numerator with no denominator change at all, so treat the rule as a prior about the *published* figure and nothing more.
- **Overall: 91%** (3530/3860 commands, 1622 tests, 21 heuristic disagreements) as measured by CI on `ubuntu-latest` for `e90e5d1a` (#258, merged as `30862618`) — this supersedes `bb7b6ab`/#257's figure above as the current reading. All four read from run 34438891646 (`bash-coverage`, job 102754979498; `Test count` from job 102755003116) in a single pass: `TOTAL 3530 3860 91%`, `total: 21`, `Test count: 1622`. **This branch produced four full-gate runs and no two agree on the ratio** — spike worktree `3542/3857`, macOS `3540/3859`, the Linux 7950X `3539/3860`, CI `3530/3860`. The union denominator depends on what each run actually traced, so the ratio is one sample rather than a constant; **91% is the durable claim and the fraction is not**. Local read HIGHER than CI on both machines here (3540 and 3539 against 3530 covered, on an identical or near-identical denominator), which is the local-reads-one-point-higher prior holding in numerator terms without crossing a display boundary. **The first CI attempt on this SHA failed and it was a flake**: `bash-coverage` went red on `not ok 341 extract_new_content: no false positives when state is 20KB-truncated subset of current`, the red-suite guard correctly refused to compute coverage over it, and the identical commit passed on re-run. That test passed in isolation on macOS and on the workstation with the tracer both on and off, and passed in all three full-suite runs off CI — so treat a lone red there as a re-run candidate before a regression.
- **Overall: 91%** (3530/3860 commands, 1626 tests, 21 heuristic disagreements) as measured by CI on `ubuntu-latest` for `971d789f` (#259, merged as `9a7a2a0c`) — this supersedes `e90e5d1a`/#258's figure above as the current reading. All four read from run 34500582029 (`bash-coverage`, job 102950061164; `Test count` from job 102950060886) in a single pass: `TOTAL 3530 3860 91%`, `total: 21`, `Test count: 1626`. **The coverage triple is byte-identical to #258's and that is expected, not a copied figure:** #259 changed no instrumented file — `.config/.zshrc.d/6_path.zsh` is zsh and outside the bash set, and the other three files are a `.bats` file and two Markdown files — so four new tests moved the count 1622 -> 1626 and the ratio not at all. No local coverage preview was taken, so this is neither a confirmation nor a counter-example to the local-reads-one-point-higher rule.
- **Overall: 91%** (3530/3860 commands, 1626 tests, 21 heuristic disagreements) as measured by CI on `ubuntu-latest` for `1d309276` (#260, merged as `ff66c68e`) — this supersedes `971d789f`/#259's figure above as the current reading. All four read from run 34520947214 (`bash-coverage`, job 103018029283; `Test count` from job 103018029320) in a single pass: `TOTAL 3530 3860 91%`, `total: 21`, `Test count: 1626`. **Byte-identical to #259's triple, and expected:** #260 changed one `.bats` assertion plus Markdown, so no instrumented file moved and no test was added or removed. No local coverage preview was taken, so this is neither a confirmation nor a counter-example to the local-reads-one-point-higher rule.
...
- **91% has now landed exactly at the floor on the fifth consecutive CI measurement** — `21671b8`, `346d25f`, `4a04ea7`, and now `25d41f1` (#239), each time with the local preview one point higher where one was taken. Five instances is enough to stop treating "local reads one point high" as a tendency and start treating it as the rule: **publish CI's number, never a local one, and expect the CI figure to sit on the floor rather than above it.** A change that adds instrumented lines without tests will therefore breach the gate immediately rather than eroding a margin, because there is no margin.
...
- **The denominator is the union of the static heuristic's coverable-line count and whatever the trace file actually contains for that file, never the heuristic alone.** This is what makes `covered <= coverable` hold by construction rather than by luck — every traced line is by definition a member of the union, so it can never exceed it. A wrongly-excluded line therefore raises the denominator (and the numerator, since the trace hit it) rather than silently lowering the percentage; an over-matching exclusion rule can never manufacture a higher score. Each run prints every union-added line as a **heuristic disagreement** — the heuristic said not-coverable, the trace disagreed — so a systematically wrong exclusion rule stays visible instead of being absorbed into a bigger denominator. The current CI run reports 18, down from 192 before the heredoc/continuation rules landed; a non-zero count is a to-do against the heuristic, not a failure of the gate. It rose from 15 when `ZSH_FILES` split the lint scope — widening what the suite executes surfaces more traced lines, so this count moves with coverage and is not a fixed target. **Read it beside the ratio from the same run, never a ratio from another one.** A union-added line raises numerator and denominator together, so a disagreement count that moves while `3159/3442` sits still is not a real anomaly — it is two figures recorded at different times. That happened here: the count above and the ratio in the Coverage section are now both from `5e1f934`.
