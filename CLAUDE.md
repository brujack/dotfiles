...
- Force the serial path anywhere with `make test HAVE_PARALLEL=`. The bats file list is a filesystem walk, not `git ls-files` (so untracked `.bats` files still run); add a file to `BATS_SERIAL_FILES` only alongside a measurement showing it fails under `--jobs`. → `dotfiles-testing-toolchain.md` § `make test parallel jobs: detection, JOBS validation, and the CI file list`
...
- Never put provenance (a `uv.lock` SHA) in a rendering's header — a `runtime`-group edit moves `uv.lock` without changing `test-lint`'s export, so a provenance header would force a re-render on every unrelated change. Provenance belongs on a consumer's own copy, written at copy time. → `dotfiles-testing-toolchain.md` § `Requirements CI groups: purpose over CI/local, and the erosion guard`
...
- In tests, set `_OVERRIDE_VAR` to a writable temp copy; leave it unset in production code. → `dotfiles-test-seams.md` § `Test seam idiom and override pattern`
...
- `_OVERRIDE_CURRENT_LOGIN_SHELL` (`lib/helpers.sh:_current_login_shell`)
  ...
  - Check both `chsh`'s and `sudo -n chsh`'s exit codes rather than logging "Changed default shell" unconditionally — `chsh` authenticates via PAM and exits 1 non-interactively, so an unchecked rc reports success over an unchanged shell. → `dotfiles-test-seams.md` § `_OVERRIDE_CURRENT_LOGIN_SHELL seam and the chsh guard`
...
- `_AWS_GPG_BIN`/`_AWS_PKGUTIL_BIN`/`_AWS_KEY_PATH` (`lib/developer.sh:_aws_verify_zip`/`_aws_verify_pkg`, `lib/helpers.sh:_doctor_check_aws_key_expiry`)
  ...
  - A suite sourcing via an absolute path (bats' `load_setup_env`) can't exercise this cwd-sensitivity; regression tests must `source ./lib/...` relatively from the repo root, then `cd` away before asserting. → `dotfiles-test-seams.md` § `_AWS_GPG_BIN / _AWS_PKGUTIL_BIN / _AWS_KEY_PATH seams`
...
- `PATH` (`cadence.plist.template`, `lib/launch_agents.sh`)
...
- `NTFY_URL`/`NTFY_TOPIC` (`scripts/cadence-notify.sh:_rhn_notify`)
...
- For the full `MOCK_*` env var reference table and the general mock usage pattern, use this file's own `Mock Pattern` section below rather than looking elsewhere — this heading exists only to receive the pointer. → `dotfiles-bats-test-infrastructure.md` § `Mock Pattern full reference pointer (superseded by this section)`
...
- Add both edits together in the same commit. → `dotfiles-conventions.md` § `_UPDATE_SECTION_ORDER coupling`
...
- The `[[ ! -x "${_root}/bin/tfenv" ]]` guard must run and return before the symlink loop: running the loop first plants two dangling symlinks that the guard's own `-L` branch then treats as already repaired. The remedy for a broken checkout (`rm -rf ${_root}`) belongs in the WARN, not in doctor. → `dotfiles-conventions.md` § `terraform on Linux via tfenv, and the checkout guard`
