...
The pre-push hook is **permanent**. It runs `make test` (lint + bats) on every push before the push reaches GitHub, but only when the push includes changes to `.sh` files, `.bats` files, the root `Makefile`, or anything under `scripts/` or `tests/`. Pushes touching only config files (`.toml`, `.json`, `.md`, etc.) outside `scripts/` and `tests/` skip the test run entirely. Skips branch deletions. This conserves GitHub Actions minutes — CI runs only on PRs.
...
- `test` job: installs bats + shellcheck, runs `make test`, then verifies test count ≥ 840 (regression proxy; 1103 tests as of 2026-07-31)
...
- **Overall: 91%** (1103 tests as of 2026-07-31); gated in CI at 90% (`bash-coverage` job, blocks auto-merge on drop).
