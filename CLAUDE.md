...
- **The awscli signature-verification branch has only a local macOS preview, not yet a CI-measured figure — per the rule above, that preview does not replace `b5e01e6b`'s number until CI re-measures on this PR.** Measured locally: 91% (3551/3880 commands), 19 heuristic disagreements, 1641 tests. `lib/developer.sh` grew from 216 to 300 coverable lines (94%, 283/300 covered) — the largest single contributor to the denominator change, and consistent with `#244`/`#250` above: new platform-conditional verification branches usually widen CI's count more than macOS's local one.
