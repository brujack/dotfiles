...
- `_OVERRIDE_CURRENT_LOGIN_SHELL` (`lib/helpers.sh:_current_login_shell`)
  ...
  - The three end-to-end `run_doctor` tests stub every sub-check by name; stub `_doctor_check_login_shell` there too, or it reads the real account mid-suite. → `dotfiles-test-seams.md` § `_OVERRIDE_CURRENT_LOGIN_SHELL seam and the chsh guard`
