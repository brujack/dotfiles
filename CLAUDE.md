...
**CI takes the parallel path too, and not because the workflow asks for it.** `ubuntu-latest` preinstalls GNU parallel, so `HAVE_PARALLEL` resolves to `yes` on the runner whatever the workflow installs — a claim to the contrary read off `ci.yml` is reading a file with no field for what the image ships (ai-config ADR-0078 Amendment 2). Measured there on the same runner class: serial 324s median over 15 runs, `--jobs $(nproc)` = 2 workers 374s, `--jobs 12` 279s. Never pass `JOBS="$(nproc)"`: on a 2-vCPU runner it is the worst available setting. To force serial anywhere, `make test HAVE_PARALLEL=`.
