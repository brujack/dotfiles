...
if [[ -n ${SETUP} ]] || [[ -n ${DEVELOPER} ]]; then
  ...
  if [[ -n ${MACOS} ]]; then
    ...
    if ! [ -x "$(command -v brew)" ]; then
      ...
      if [[ -n ${STUDIO} ]] || [[ -n ${LAPTOP} ]] || [[ -n ${RECEPTION} ]] || [[ -n ${RATNA} ]]; then
...
EOM
  ...
  if [[ -n ${LAPTOP} ]] || [[ -n ${STUDIO} ]] || [[ -n ${RECEPTION} ]] || [[ -n ${RATNA} ]]; then
...
if [[ -n ${DEVELOPER} ]] || [[ -n ${ANSIBLE} ]]; then
  ...
  if ! [[ $(readlink "${HOME}/.pyenv/versions/ansible") == "${HOME}/.pyenv/versions/${PYTHON_VER}/envs/ansible" ]]; then
    if [[ -n ${STUDIO} ]] || [[ -n ${LAPTOP} ]] || [[ -n ${RECEPTION} ]] || [[ -n ${WORKSTATION} ]] || [[ -n ${CRUNCHER} ]] || [[ -n ${RATNA} ]]; then
