...
setup_ansible() {
  ...
  if ! [[ $(readlink "${HOME}/.pyenv/versions/ansible") == "${HOME}/.pyenv/versions/${PYTHON_VER}/envs/ansible" ]]; then
    if [[ -n ${HAS_DEVTOOLS} ]]; then
      ...
      python -m pip install ansible ansible-lint certbot certbot-dns-cloudflare checkov boto3 docker gmpy2 jmespath mpmath netaddr pylint psutil bpytop HttpPy j2cli wheel shell-gpt pyright mutmut hypothesis mlx
    fi
  fi
}
...
recreate_python_venv() {
  ...
  if [[ "${_venv_name}" == "ansible" ]]; then
    ...
    printf "Installing Ansible dependencies...\\n"
    "${_python}" -m pip install ansible ansible-lint certbot certbot-dns-cloudflare checkov boto3 docker gmpy2 jmespath mpmath netaddr pylint psutil bpytop HttpPy j2cli wheel shell-gpt pyright mutmut hypothesis mlx || return 1
  fi
}
