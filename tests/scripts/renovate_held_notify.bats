...
setup() {
  ...
  SCRIPT="${REPO_ROOT}/scripts/renovate-held-notify.sh"
...
@test "exit 0 with nothing held sends no notification and still writes the heartbeat" {
  ...
  [ "$status" -eq 0 ]
  [ ! -s "${_RHN_NTFY_LOG}" ]
  [ -f "${_RHN_STATE_DIR}/last-run.json" ]
  command grep -q '"result": *"clean"' "${_RHN_STATE_DIR}/last-run.json"
}
...
@test "exit 1 with held PRs notifies and names each finding" {
  ...
  command grep -q '"result": *"held"' "${_RHN_STATE_DIR}/last-run.json"
}
...
@test "exit 2 is reported as incomplete, never as clean" {
  ...
  command grep -q '"result": *"incomplete"' "${_RHN_STATE_DIR}/last-run.json"
  ! command grep -q '"result": *"clean"' "${_RHN_STATE_DIR}/last-run.json"
}
...
@test "running twice leaves one heartbeat, refreshed" {
  ...
  local _first; _first=$(command grep -o '"ts": *"[^"]*"' "${_RHN_STATE_DIR}/last-run.json")
  run bash "${SCRIPT}"; [ "$status" -eq 0 ]
