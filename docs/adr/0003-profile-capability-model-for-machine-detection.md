...
New capability-gated code checks `HAS_*` vars, not raw hostnames. Legacy hostname vars (`LAPTOP`, `STUDIO`, etc.) are preserved as readonly aliases in `detect_env.sh` for backwards compatibility but are not used in new code.
