...
| Feature                                                                             | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Git-hook self-coverage gap                                                          | `scripts/pre-push`'s own trigger regex `'\.(sh|bats)$|^Makefile$|^tests/'` does not match extensionless `scripts/pre-push` or `scripts/commit-msg`, so the hook does not test itself — an edit to either pushes without running the suite. CI backstops it. Fix is one character class (`|^scripts/`). Pairs with the git-env hygiene follow-up (15 hand-copied unsets). Found during dotfiles#191 bug-scan.                                                                                                                                                                                                                                                                                                                                                     |
...
| hooksPath detector: unreadable scope reads as unset                                 | `_git_hooks_hookspath_offenders`'s `|| continue` treats a scope git could not read at all (rc 128 — e.g. a malformed config, or `/etc/gitconfig` unreadable while `--global` reads fine) identically to "key unset", so that scope is reported clean. Deferred during dotfiles#194 on the grounds that a corrupt gitconfig makes essentially every other git call fail loudly at the same moment — which does NOT cover the scope-specific case above. Fix is to capture rc and branch: rc 1 = clean, rc > 1 = a distinguishable "cannot determine" record, plus the matching error-path test. |
