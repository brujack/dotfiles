...
| 2026-07-29 | doctor-hookspath-check                                                                     | [spec](specs/2026-07-29-doctor-hookspath-check-design.md)           | Pending |
...
| Surface git-hooks gap counts in the `-t update` summary block                       | Raised as a WARNING by `pr-review` on #189 and accepted, not fixed. `_update_record_end` has no `git-hooks)` case arm, so the section renders as a bare `[OK] git-hooks updated` even when the sweep found gaps — the actionable line (`N checked, M updated, K gaps (names)`) reaches `log_info` and `~/.dotfiles-update.log` but not the summary table a weekly reader scans. The structurally analogous `git-repos` and `legacy-rsync` sections both call `_update_warn` + `_update_write_detail_from_err` for their partial-success state, and the sweep has an exactly analogous one (gaps>0 or unknown>0 with rc 0). Small, self-contained, and it closes the gap between the design's stated actionability goal and where the information actually lands. |
