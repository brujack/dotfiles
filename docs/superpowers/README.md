...
| 2026-07-31 | [pre-push-self-coverage](plans/2026-07-31-pre-push-self-coverage.md) | [spec](specs/2026-07-31-git-hook-detection-gaps-design.md) | In Progress |
...
| hooksPath detector blind to include-borne pins | `_git_hooks_hookspath_offenders` reads each scope with `git config --<scope> --get`, which defaults to `--no-includes`; effective hook resolution traverses includes. Measured: an `[include]` setting `core.hooksPath` reads rc 1 with **empty stderr** — byte-identical to clean-unset — while `rev-parse --git-path hooks` returns the pin and `git status` stays rc 0. A fully effective machine-wide hook redirect renders as `[PASS] <scope>: unset`. Live since dotfiles#194; not hypothetical here, `.gitconfig_linux:30` and `.gitconfig_mac_gitlab:38` both use `[includeIf]`. Scope-level `--unset` remedy also fails against it (rc 5, pin survives) — remedy must name the origin file. Fix direction: `--includes --show-origin`. Full reproduction and five adjacent measured facts in [`ai-config/docs/knowledge/dotfiles-bug-hunt-leads.md`](https://github.com/brujack/ai-config/blob/master/docs/knowledge/dotfiles-bug-hunt-leads.md). Split out of the 2026-07-31 pre-push spec after two rounds of lens review. |
