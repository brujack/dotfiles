...
| `npm install json2yaml` is missing `-g`                                             | `lib/workflows.sh` in `run_developer_or_ansible` installs json2yaml **into the current working directory** instead of globally, so it lands wherever setup happened to run and is not on `PATH`. Deliberately left out of PR dotfiles#192 (pinning) — different bug shape. Fix is `-g` plus a version pin alongside the other three, and a test asserting the flag. |
