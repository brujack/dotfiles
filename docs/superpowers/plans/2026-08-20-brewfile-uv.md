...
> **Reachability correction, 2026-08-20 — installed and reachable are different claims, and Task 3 only checked the first.** Raised by the ai-config session and confirmed here. On the Linux workstation `uv` resolves **only from interactive zsh**: `bash -lc` -> NOT-ON-PATH, bare `ssh <cmd>` -> NOT-ON-PATH, interactive zsh -> `/home/linuxbrew/.linuxbrew/bin/uv`. The Studio is unaffected (`bash -lc` -> `/opt/homebrew/bin/uv`) because `/opt/homebrew/bin` is in `/etc/paths`, which `path_helper` feeds to login shells; linuxbrew's prefix has no such route and is added only by `6_path.zsh:55`, which interactive zsh alone sources. Task 3's own verification was circular — it exported the prefix into `PATH` and then confirmed a binary at that prefix was reachable, which is `behavior.md`'s "a check derived from the same decision as the thing it checks". Narrower than it first appears: the operator's `update` alias (`4_aliases.zsh:11`) exists only in interactive zsh, so the normal path resolves `uv` today; the exposed actors are cron, scripts and bare `ssh`. `run_update` has **0** `uv` references at this commit, so there is nothing to repair here yet — the absolute-path resolution belongs in the change that adds the `uv sync` call site. The PATH repair is backlogged separately.
> **Second reachability correction, same day — the table above was measured from a polluted environment and two of its rows were wrong.** Raised by ai-config, confirmed here with `env -i HOME=$HOME <shell> -lc`. A shell spawned from an agent session inherits the operator's interactive PATH, so neither of us was measuring a login shell — we were measuring our own environment wearing one. Corrected: Studio `zsh -lc` -> `/opt/homebrew/bin/uv`, Studio `bash -lc` and `sh -lc` -> NOT-FOUND, workstation all three -> NOT-FOUND. The `/etc/paths`/`path_helper` explanation was **wrong**: `/opt/homebrew/bin` is in neither `/etc/paths` nor `/etc/paths.d/*`. The real mechanism is `.zprofile:7`, `export PATH="/opt/homebrew/bin:$PATH"`, macOS branch only — which covers login **zsh** and nothing else, exactly as the bash row shows. So the Studio is not immune, it is immune-for-zsh, and `sh`/`bash` actors such as git hooks and cron are exposed there too. This is the third revision of this one attestation; each was measured, and the first two were measured in the wrong environment. **The environment a check runs in is part of the check.**
